...
struct DSU {
    ...
    vector < int > par, Gsize, Max;
    int components;

    DSU(int n = 0) : par(n + 5), Gsize(n + 5), Max(n + 5), components(n) {
        for(int i = 1; i <= n; i++)
            par[i] = Max[i] = i, Gsize[i] = 1;
    }
    ...
    bool union_sets(int u, int v){
        ...
        components--;
        Max[leader_u] = max(Max[leader_u], Max[leader_v]);
        return true;
    }
    ...
    // get the maximum node in the set of u
    int get_max(int u){
        return Max[find_set(u)];
    }
...
void Solve(){
    ...
    int edges_to_add = 0;
    for(int u = 1; u <= n; u++){
        int max_node = dsu.get_max(u);
        for(int v = u; v <= max_node; v++){
            if(dsu.union_sets(u, v))
                edges_to_add++;
            max_node = max(max_node, dsu.get_max(v));
        }
        u = max_node;
    }
    cout << edges_to_add << '\n';
